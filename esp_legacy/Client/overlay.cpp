...
bool isPressed(uint32_t button_code) { return false; }
...
void Overlay::RenderMenu() {
  ...
  g_settings.game_fps = g_esp_data.game_fps();
  g_settings.calc_game_fps = true;

  if (g_esp_data.aimbot().aim_mode() > 0) {
    aim_enable = true;
    if (g_esp_data.aimbot().aim_mode() > 1) {
      vis_check = true;
    } else {
      vis_check = false;
    }
  ...
  if (ImGui::CollapsingHeader("Main Toggle Settings")) {
    ...
    ImGui::SameLine();
    ImGui::TextColored(GREEN, "%.f", g_esp_data.aimbot().max_fov());
    ImGui::Dummy(ImVec2(0.0f, 10.0f));
  ...
  //}
  ImGui::Dummy(ImVec2(0.0f, 10.0f));
  ImGui::Text("%s%d%s%d", xorstr_("held="), g_esp_data.aimbot().held_id(),
              xorstr_(", weapon="), g_esp_data.aimbot().weapon_id());
  ImGui::Dummy(ImVec2(0.0f, 5.0f));
...
void Overlay::RenderInfo() {
  ...
  float aimbot_fov = g_esp_data.aimbot().max_fov();
  bool aimbot_locked = g_esp_data.aimbot().target_locked();
  bool aimbot_gun_safety = g_esp_data.aimbot().gun_safety();
  bool aimbot_grenade = g_esp_data.aimbot().held_grenade();
  int aimbot_mode = g_esp_data.aimbot().aim_mode();
...
int Overlay::CreateOverlay() {
  ...
  uint32_t screen_width = g_esp_settings.screen_width();
  uint32_t screen_height = g_esp_settings.screen_height();
  GLFWwindow *window = glfwCreateWindow(screen_width, screen_height,
                                        xorstr_("Client ImGui GLFW+OpenGL3"),
...
#include "font.h"
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // to create a named window.
    {
      ...
      float game_fps = g_esp_data.game_fps();
      if (game_fps > 0.0) {
        ImGui::SameLine();
        ImGui::Text(xorstr_(" Game(%.1f FPS)"), game_fps);
      }
      ImGui::Dummy(ImVec2(0.0f, 5.0f));
      if (esp_teammates_damage.size() > 0) {
        int current_item = 0;
        ImGui::ListBox(
            xorstr_("Damage"), &current_item,
            ...
            reinterpret_cast<void *>(&esp_teammates_damage),
      ...
      if (esp_spec_names.size() > 0) {
        int current_item = 0;
        ImGui::ListBox(
            xorstr_("Spectators"), &current_item,
            [](void *vec, int idx, const char **out_text) {
              std::vector<std::string> *vector =
                  reinterpret_cast<std::vector<std::string> *>(vec);
              if (idx < 0 || idx >= vector->size())
            ...
            reinterpret_cast<void *>(&esp_spec_names), esp_spec_names.size());
      } else {
