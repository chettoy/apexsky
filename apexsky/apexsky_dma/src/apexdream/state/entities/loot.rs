...
impl Entity for LootEntity {
    ...
    fn update(&mut self, api: &mut Api, ctx: &UpdateContext) {
        ...
        if let Ok(fields) = api.vm_gatherd(self.entity_ptr, self.entity_size, &mut indices) {
            ...
            // self.bits = sdk::HighlightBits::from_uint(fields.highlight[0]);
            // self.color[0] = f32::from_bits(fields.highlight[1]);
            // self.color[1] = f32::from_bits(fields.highlight[2]);
            // self.color[2] = f32::from_bits(fields.highlight[3]);
