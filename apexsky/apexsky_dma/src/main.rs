...
fn init_logger(non_blocking: NonBlocking, print: bool) {
    let filter_layer = EnvFilter::try_from_default_env()
        .or_else(|_| {
            EnvFilter::try_new(s!(
                "apexsky_dma=warn,apexsky=warn,apexsky_dma::mem=warn,apexsky_dma::workers::access=info,apexsky_dma::workers::aim=warn,apexsky_dma::workers::actions=warn,apexsky_dma::apexdream=warn"
            ))
        })
        .unwrap();
