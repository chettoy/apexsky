...
menu-item-toggle-no-recoil = 后坐力减弱
...
info-weapon-model-glow = 
    ...
    被队友观战时 => 绿色
    被他人观战时 => 红色
menu-item-toggle-overlay = 外部ESP服务
menu-value-no-overlay = 关闭
menu-value-external-overlay = 启动
...
menu-item-open-main-menu = 打开主菜单
menu-item-manage-dlc = 管理Mods
menu-item-creative-workshop = 创意工坊
info-coming-soon = 敬请期待
dlc-field-label = 名称
dlc-field-description = 描述
dlc-field-package-name = 包名
dlc-field-version-name = 版本
dlc-field-debug = 调试版本
dlc-field-permissions = 所需权限
dlc-installed-version = 已安装版本
dlc-not-installed-version = 未安装版本
label-dlc-install = 安装
label-dlc-uninstall = 卸载
label-dlc-running = 正在运行
label-dlc-stopped = 已停止
dlc-list-label-separator = {" | "}
dlc-list-label-quit = (Esc) 退出
dlc-list-label-move-up = (↑)
dlc-list-label-move-down = (↓)
dlc-list-label-install = (E) 安装
dlc-list-label-uninstall = (E) 卸载
dlc-list-label-reload = (R) 重新加载
info-dlc-install-success = 安装成功
info-dlc-uninstall-success = 卸载完成
info-dlc-reload-success = 已重新加载
permission-field-access-game-world = 访问游戏世界数据
permission-field-access-game-input = 访问游戏按键输入
permission-field-access-game-memory = 读取游戏内存
permission-field-access-global-settings = 读取全局设置
permission-field-bluetooth = 控制蓝牙设备
permission-field-camera = 使用相机
permission-field-manage-global-settings = 管理全局设置
permission-field-internet = 完全的网络访问权限
permission-field-modify-memory = 改动游戏内存
permission-field-overlay = 显示覆盖层
permission-field-query-all-packages = 获取已安装mod列表
permission-field-record-audio = 录音
permission-field-storage = 读写任何文件
permission-field-send-aimbot-actions = 发送Aimbot指令
permission-field-send-input-actions = 发送输入指令
