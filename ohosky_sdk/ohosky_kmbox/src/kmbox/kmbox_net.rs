...
impl KmboxNet {
    ...
    async fn recv_packet_head(&self) -> Result<CmdHead, KmboxError> {
        ...
        match timeout(
            ...
            self.socket.recv_from(rx_head.as_mut_bytes()),
        )
        .await
        {
            Ok(r) => match r {
        ...
        match timeout(Duration::from_secs(10), self.socket.recv_from(&mut buf)).await {
            Ok(r) => match r {
                Ok((_number_of_bytes, _src_addr)) => {
                    CmdHead::read_from_bytes(&buf[0..size_of::<CmdHead>()])
                        .map_err(|e| KmboxError::AnyError(anyhow::anyhow!("recv head {:?}", e)))
                }
                Err(e) => Err(KmboxError::NetRx(e)),
            },
            Err(e) => Err(KmboxError::NetRxTimeout(e)),
    ...
    async fn send_packet(
        ...
        self.tx_packet = ClientPacket {
            head: CmdHead {
                ...
                rand: rand_data.unwrap_or_else(rand::random),
